...
def _combine_product_rule(mlp_u, mlp_J, mlp_H,
                          ...
                          p: int):
    N = mlp_u.shape[0]
    dim_Y = mlp_u.shape[1] - 1  # Dynamically extract n for H^{n+1}
    ...
    term_A = p * rho_pm1_bc * nn_k.view(N, dim_Y, 1) * rho_J.view(N, 1, 2)
    ...
    nn_k_bc = nn_k.view(N, dim_Y, 1, 1)
    rho_H_bc = rho_H.view(N, 1, 2, 2)

    A = (nnJ_k.view(N, dim_Y, 1, 2) * rho_J.view(N, 1, 2, 1)
         + nnJ_k.view(N, dim_Y, 2, 1) * rho_J.view(N, 1, 1, 2))  
...
def minimal_in_Hn_PDE_flat(u_callable, use_compile=True, compile_kwargs=None):
